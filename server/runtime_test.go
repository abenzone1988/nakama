...
nakama.register_rpc(test.printWorld, "helloworld")`,
	...
	apiServer := StartApiServer(logger, logger, db, protojsonMarshaler, protojsonUnmarshaler, cfg, "", nil, storageIdx, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, metrics, pipeline, runtime, nil)
	...
	payload := "\"Hello World\""
	client := &http.Client{}
	request, _ := http.NewRequest(http.MethodPost, "http://localhost:7350/v2/rpc/helloworld?http_key=defaulthttpkey", strings.NewReader(payload))
