...
func (s *ApiServer) GainChallengeReward(ctx context.Context, in *game.GainChallengeRewardRequest) (*game.GainChallengeRewardResponse, error) {
	...
	activityInfo, found := s.template.GetTplChallengeInfo().FindByKey(tplChallenge.ActivityID)
	if !found {
	...
	if in.RankReward {
		...
		// 获取排名奖励配置
		acRewards := s.template.GetTplChallengeReward().FindByFilter(func(acReward template.TplChallengeReward) bool {
...
// 检查积分等级奖励（公共逻辑）
func (s *ApiServer) checkScoreRewards(challengeData *ChallengeStatus, ownerRecord *api.LeaderboardRecord, activityInfo *template.TplChallengeInfo) *ScoreRewardResult {
	...
	for _, challenge := range userMatch.Challenges {
		if challenge.TournamentID != "" {
			...
			if now.After(overTime) {
				...
				// 获取活动配置
				activityInfo, found := s.template.GetTplChallengeInfo().FindByKey(challenge.ActivityID)
				if !found {
...
// 发送过期的排名奖励
func (s *ApiServer) sendExpiredRankReward(ctx context.Context, userID uuid.UUID, challenge *ChallengeStatus, ownerRecord *api.LeaderboardRecord, activityInfo *template.TplChallengeInfo) (bool, error) {
	// 获取排名奖励配置
	acRewards := s.template.GetTplChallengeReward().FindByFilter(func(acReward template.TplChallengeReward) bool {
		return acReward.ActiveID == activityInfo.RewardGroupID
	})
...
// 发送过期的积分等级奖励
func (s *ApiServer) sendExpiredScoreRewards(ctx context.Context, userID uuid.UUID, challenge *ChallengeStatus, scoreResult *ScoreRewardResult, activityInfo *template.TplChallengeInfo) error {
	// 解析奖励
