...
type TableTplEquipmentBase struct {
	logger    *zap.Logger
	loadPath  string
	tableData map[int32]TplEquipmentBase
}
...
func NewTableTplEquipmentBase(logger *zap.Logger, loadPath string) *TableTplEquipmentBase {
	return &TableTplEquipmentBase{
		logger:    logger,
		loadPath:  loadPath,
		tableData: make(map[int32]TplEquipmentBase),
	}
}

func (t *TableTplEquipmentBase) FindByKey(key int32) (TplEquipmentBase, bool) {
...
func DeserializeStringToTplEquipmentBaseMap(jsonStr []byte, logger *zap.Logger) map[int32]TplEquipmentBase {
	if len(jsonStr) == 0 {
		return make(map[int32]TplEquipmentBase)
	}
	...
	result := make(map[int32]TplEquipmentBase)
